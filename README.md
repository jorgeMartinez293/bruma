...
grande; bruma usa **WKWebView** = el WebKit del sistema → RAM baja, binario diminuto, y
...
`bruma.app` es una *agent app* (`LSUIElement`): no aparece en el Dock, solo un icono en
la barra de menú. Para que arranque al iniciar sesión: Ajustes del Sistema → General →
Ítems de inicio → añadir `bruma.app`.
